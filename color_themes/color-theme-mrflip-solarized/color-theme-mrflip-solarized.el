;;; color-theme-mrflip-solarized.el --- A version of Ethan Schoonover's Solarized themes

;; Copyright (C) 2011-2014 Steve Purcell

;; Author: Steve Purcell <steve@mrflip.com>
;; Keywords: faces themes
;; Homepage: http://github.com/purcell/color-theme-mrflip-solarized
...
;;     M-x color-theme-mrflip-solarized-light
;;     M-x color-theme-mrflip-solarized-dark
;;
;;; Credit:
...
(defgroup color-theme-mrflip-solarized nil
  "The mrflip solarized theme pair."
  :group 'appearance
  :prefix "color-theme-mrflip-solarized-")

(defcustom color-theme-mrflip-solarized-rgb-is-srgb
...
Set this to t if using the sRGB patch on OS X."
  :type 'boolean
  :group 'color-theme-mrflip-solarized)
...
'((base03    "#002b36" "#042028" "#1c1c1c" "brightblack"   "black")
  ...
  (green     "#298932" "#728a05" "#5f8700" "green"         "green")
  (baseT     "#eee8d6" "#e9e2cb" "#e4e4e4" "white"         "white")
  (baseH     "#f5f3e4" "#fcf4dc" "#ffffd7" "brightwhite"   "white")
  (baseC     "#d3ceb9" "#e9e2cb" "#e4e4e4" "white"         "white")
  (pinkish   "#faeedc" "#c60007" "#d70000" "red"           "red")
  (blueish   "#d4e9fa" "#2075c7" "#0087ff" "blue"          "blue")
  (pinkish0  "#433d18" "#c60007" "#d70000" "red"           "red")
  (blueish0  "#183d43" "#2075c7" "#0087ff" "blue"          "blue")
  (greenish  "#e7fde4" "#728a05" "#5f8700" "green"         "green")
  (darkgreen "#016731" "#728a05" "#5f8700" "green"         "green")
  )

(defmacro color-theme-mrflip-solarized--with-colors (mode &rest body)
...
`MODE' should be set to either 'light or 'dark."
  ...
  `(let* ((srgb color-theme-mrflip-solarized-rgb-is-srgb)
          ...
          (green   (if srgb "#298932" "#298932")) ; (0.4498 0.5412 0.0202)
          ;;
          (baseT     "#eee8d6")
          (baseH     "#f2f0e0") ;; f5f3e4
          (baseC     "#d3ceb9")
          ...
          (darkgreen "#016731")
          ;;
          (foregrounds          (list base00  base01 base03 base02))
          (backgrounds          (list baseH  base02))
...
(defmacro color-theme-mrflip-solarized--face-specs ()
  ...
  (quote
   (mapcar
    ...
    `(;; Standard font lock faces
      ...
      (italic                                   (:slant italic))
      (nobreak-space                            (:background ,base2))
      ...
      (font-lock-function-name-face             (:foreground ,blueish0))
      ...
      ;; Flycheck
      (flycheck-error                           (:underline (:style wave :color ,pinkish)))
      (flycheck-info                            (:underline (:style wave :color ,greenish)))
      (flycheck-warning                         (:underline (:style wave :color ,blueish)))
      ...
      (diff-added                               (:foreground ,greenish :background ,base0))
      (diff-changed                             (:foreground ,blueish))
      (diff-removed                             (:foreground ,pinkish))
      (diff-context                             (:foreground "#eeeede"))
...
(defmacro color-theme-mrflip-solarized--frame-parameter-specs ()
...
(defmacro color-theme-mrflip-solarized--define-theme (mode)
  "Define either the dark or the light theme.
Argument MODE: 'light or 'dark"
  (let ((name (intern (format "mrflip-solarized-%s" (symbol-name mode))))
        (doc (format "A version of Ethan Schoonover's 'Solarized' theme (%s version)" mode)))
    `(progn
       (deftheme ,name ,doc)
       (put ',name 'theme-immediate t)
       (color-theme-mrflip-solarized--with-colors
        ',mode
        (apply 'custom-theme-set-faces ',name
               (color-theme-mrflip-solarized--face-specs))
...
(defun color-theme-mrflip-solarized (mode)
  "Apply either the dark or the light theme."
  (if (fboundp 'load-theme)
      (let ((name (cond
                    ((eq 'light mode) 'mrflip-solarized-light)
                    ((eq 'dark mode) 'mrflip-solarized-dark)
    ...
    (progn
      ...
      (color-theme-mrflip-solarized--with-colors
       mode
       (color-theme-install
        `(,(intern (concat "color-theme-mrflip-solarized-" (symbol-name mode)))
          ,@(color-theme-mrflip-solarized--frame-parameter-specs)
          ,@(color-theme-mrflip-solarized--face-specs)))
...
;;;###autoload
(defun color-theme-mrflip-solarized-dark ()
  "Apply the dark solarized theme."
  (interactive)
  (color-theme-mrflip-solarized 'dark))

;;;###autoload
(defun color-theme-mrflip-solarized-light ()
  "Apply the light solarized theme."
  (interactive)
  (color-theme-mrflip-solarized 'light))


(provide 'color-theme-mrflip-solarized)

;; Local Variables:
;; byte-compile-warnings: (not cl-functions)
;; End:

;;; color-theme-mrflip-solarized.el ends here
